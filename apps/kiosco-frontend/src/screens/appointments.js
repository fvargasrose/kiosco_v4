 ...
 * En cada cita próxima se ofrece cancelar/reagendar (que en el Hito 6 redirige
 * a una vista informativa: para procesar la acción se requiere backend del
 * Hito 7, así que aquí solo recomendamos contactar recepción).
 */
...
export function renderAppointments(container, _params, navigate) {
  ...
  const loadList = async (status) => {
    ...
    try {
      ...
      renderList(listContainer, res.data ?? [], status);
    } catch (err) {
...
function renderList(container, items, status) {
  ...
  container.querySelectorAll('.appointment-action').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (!apt) return;
      handleAppointmentAction(apt, btn.dataset.action);
    });
  });
}
...
function handleAppointmentAction(apt, action) {
  // Hito 6: backend de cancelar/reagendar viene en Hito 7.
  // Por ahora informamos al paciente que debe ir a recepción.
  const title = action === 'cancel' ? 'Cancelar cita' : 'Reagendar cita';
  const body =
    action === 'cancel'
      ? `Para cancelar la cita del ${formatDate(apt.fecha)} a las ${apt.hora_inicio}, por favor dirígete a recepción o llama al teléfono de la clínica. Próximamente podrás hacerlo desde el kiosco.`
      : `Para reagendar la cita del ${formatDate(apt.fecha)} a las ${apt.hora_inicio}, por favor dirígete a recepción. Próximamente podrás hacerlo desde el kiosco.`;

  showModal({
    icon: action === 'cancel' ? '⚠️' : '📅',
    title,
    body,
